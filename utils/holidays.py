...
HOLIDAYS_SORTED = np.sort(HOLIDAYS.values.astype("datetime64[D]"))
...
def add_holiday_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    date_vals = out["Date"].values.astype("datetime64[D]")
    pos = np.searchsorted(HOLIDAYS_SORTED, date_vals)
    ...
    # Compute deltas
    days_since_prev = (date_vals - prev_holiday).astype("timedelta64[D]").astype(float)
    days_to_next    = (next_holiday - date_vals).astype("timedelta64[D]").astype(float)

    # Replace NaNs with sentinel values
